...
public class PayPal {
	...
	public PayPal() {
		...
		try {
			String filename = "cn/yiyingli/PayPal/config.properties";
	 ...
	 *********************************************************************************/
	public Map<String, String> callShortcutExpressCheckout(Map<String, String> checkoutDetails, String returnURL,
			String cancelURL) {
	 ...
	 * Output: Returns a HashMap object containing the response from the server.
	 *********************************************************************************/
	public HashMap<String, String> callMarkExpressCheckout(Map<String, String> checkoutDetails, String returnURL,
			String cancelURL) {
		// ------------------------------------------------------------------------------------------------------------------------------------
		// Construct the parameter string that describes the SetExpressCheckout
		// API call in the shortcut implementation
		...
		nvpstr.append("&RETURNURL=").append(returnURL).append("&CANCELURL=").append(cancelURL)
				.append("&PAYMENTREQUEST_0_SELLERPAYPALACCOUNTID=").append(this.getSellerEmail());
		...
		if (isSet(checkoutDetails.get("PAYMENTREQUEST_0_INSURANCEAMT"))) {
			nvpstr.append("&PAYMENTREQUEST_0_INSURANCEAMT=")
					.append(checkoutDetails.get("PAYMENTREQUEST_0_INSURANCEAMT"));
		}
		...
		if (isSet(checkoutDetails.get("PAYMENTREQUEST_0_SHIPTOSTREET"))) {
			nvpstr.append("&PAYMENTREQUEST_0_SHIPTOSTREET=")
					.append(checkoutDetails.get("PAYMENTREQUEST_0_SHIPTOSTREET"));
		}

		if (isSet(checkoutDetails.get("PAYMENTREQUEST_0_SHIPTOSTREET2"))) {
			nvpstr.append("&PAYMENTREQUEST_0_SHIPTOSTREET2=")
					.append(checkoutDetails.get("PAYMENTREQUEST_0_SHIPTOSTREET2"));
		}
		...
		if (isSet(checkoutDetails.get("PAYMENTREQUEST_0_SHIPTOCOUNTRY"))) {
			nvpstr.append("&PAYMENTREQUEST_0_SHIPTOCOUNTRY=")
					.append(checkoutDetails.get("PAYMENTREQUEST_0_SHIPTOCOUNTRY"));
		}
		if (isSet(checkoutDetails.get("PAYMENTREQUEST_0_SHIPTOPHONENUM"))) {
			nvpstr.append("&PAYMENTREQUEST_0_SHIPTOPHONENUM=")
					.append(checkoutDetails.get("PAYMENTREQUEST_0_SHIPTOPHONENUM"));
		}
	...
	public HashMap<String, String> confirmPayment(Map<String, String> checkoutDetails, String serverName) {
		...
		if (isSet(checkoutDetails.get("currencyCodeType")))
			nvpstr.append("&PAYMENTREQUEST_0_CURRENCYCODE=")
					.append(encode(checkoutDetails.get("currencyCodeType").toString()));

		if (isSet(checkoutDetails.get("PAYMENTREQUEST_0_ITEMAMT")))
			nvpstr.append("&PAYMENTREQUEST_0_ITEMAMT=")
					.append(encode(checkoutDetails.get("PAYMENTREQUEST_0_ITEMAMT").toString()));

		if (isSet(checkoutDetails.get("PAYMENTREQUEST_0_TAXAMT")))
			nvpstr.append("&PAYMENTREQUEST_0_TAXAMT=")
					.append(encode(checkoutDetails.get("PAYMENTREQUEST_0_TAXAMT").toString()));

		if (isSet(checkoutDetails.get("shippingAmt")))
			nvpstr.append("&PAYMENTREQUEST_0_SHIPPINGAMT=")
					.append(encode(checkoutDetails.get("PAYMENTREQUEST_0_SHIPPINGAMT").toString()));

		if (isSet(checkoutDetails.get("handlingAmt")))
			nvpstr.append("&PAYMENTREQUEST_0_HANDLINGAMT=")
					.append(encode(checkoutDetails.get("PAYMENTREQUEST_0_HANDLINGAMT").toString()));

		if (isSet(checkoutDetails.get("shippingDiscAmt")))
			nvpstr.append("&PAYMENTREQUEST_0_SHIPDISCAMT=")
					.append(encode(checkoutDetails.get("PAYMENTREQUEST_0_SHIPDISCAMT").toString()));

		if (isSet(checkoutDetails.get("insuranceAmt")))
			nvpstr.append("&PAYMENTREQUEST_0_INSURANCEAMT=")
					.append(encode(checkoutDetails.get("PAYMENTREQUEST_0_INSURANCEAMT").toString()));
	...
	/*********************************************************************************
	 * httpcall: Function to perform the API call to PayPal using API
	 * signature @ methodName is name of API method. @ nvpStr is nvp string.
	...
	public HashMap<String, String> httpcall(String methodName, String nvpStr) {
		...
		try {
			...
			while (true) {
				try {
					opt = conn.getOutputStream();
					break;
				} catch (IOException e) {
					continue;
				}
			}
			DataOutputStream output = new DataOutputStream(opt);
